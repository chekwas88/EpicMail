...
var token;
(0, _mocha.describe)('GET /api/v1/groups/:id/users', function () {
  ...
  (0, _mocha.it)('it should return an error if token cannot be verifed', function (done) {
    (0, _chai.request)(_app["default"]).get('/api/v1/groups/1/users').set('authorization', 'Bearer jxxxxxxxxxxxxnns66s').end(function (err, res) {
  ...
  (0, _mocha.it)('it should return an error if token is not provided', function (done) {
    (0, _chai.request)(_app["default"]).get('/api/v1/groups/1/users').end(function (err, res) {
  ...
  (0, _mocha.it)('it gets all the  groups of a user', function (done) {
    (0, _chai.request)(_app["default"]).get('/api/v1/groups/1/users').set('authorization', "Bearer ".concat(token)).end(function (err, res) {
      _chai.assert.isArray(res.body.data);

      _chai.assert.equal(res.body.data[0].message, 'members retrieved');
  ...
  (0, _mocha.it)('it throws not found error a messages', function (done) {
    (0, _chai.request)(_app["default"]).get('/api/v1/groups/1001/users').set('authorization', "Bearer ".concat(token)).end(function (err, res) {
      if (!res.body.data) {
        _chai.assert.equal(res.body.error, 'no group member found');
