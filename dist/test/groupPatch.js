...
var token;
(0, _mocha.describe)('PATCH api/v1/groups/:id/name', function () {
  ...
  (0, _mocha.it)('it should return an error if token cannot be verifed', function (done) {
    (0, _chai.request)(_app["default"]).patch('/api/v1/groups/1/name').set('authorization', 'Bearer jxxxxxxxxxxxxnns66s').send({
      name: 'cycle1'
    }).end(function (err, res) {
  ...
  (0, _mocha.it)('it should return an error if token is not provided', function (done) {
    (0, _chai.request)(_app["default"]).patch('/api/v1/groups/1/name').send({
      name: 'cycle2'
    }).end(function (err, res) {
  ...
  (0, _mocha.it)('it updates a group', function (done) {
    (0, _chai.request)(_app["default"]).patch('/api/v1/groups/1/name').set('authorization', "Bearer ".concat(token)).send({
      name: 'cycle1'
    }).end(function (err, res) {
      _chai.assert.equal(res.body.data[0].message, 'Group updated');
  ...
  (0, _mocha.it)('it should return BadRequestError if subject is not provided', function (done) {
    (0, _chai.request)(_app["default"]).patch('/api/v1/groups/1/name').set('authorization', "Bearer ".concat(token)).send({}).end(function (err, res) {
      _chai.assert.equal(res.body.errors.name, 'Group name should be provided');
