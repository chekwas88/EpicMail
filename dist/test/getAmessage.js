...
(0, _mocha.describe)('GET api/v1/messages/:id', () => {
  ...
  (0, _mocha.it)('it should return an error if token is not provided', done => {
    (0, _chai.request)(_app2.default).get('/api/v1/messages/1').end((err, res) => {
  ...
  (0, _mocha.it)('it gets a message', done => {
    (0, _chai.request)(_app2.default).get('/api/v1/messages/1').set('authorization', `Bearer ${token}`).end((err, res) => {
